...
from .constantsGroup import ConstantsGroup
from .autofix import AtType, Suggestion, Issue, IssueSeverity
...
class BaseRuleChecker(object):
    ...
    def _AddFailedCheck(
        self, message: str, at: Optional[AtType] = None, suggestion: Optional[Suggestion] = None
    ) -> None:
        self._failedChecks.append(
            Issue(message=message, severity=IssueSeverity.FAILURE, rule=self.__class__, at=at, suggestion=suggestion)
        )
    ...
    def _AddError(
        ...
        self._errors.append(
            Issue(message=message, severity=IssueSeverity.ERROR, rule=self.__class__, at=at, suggestion=suggestion)
        )
    ...
    def _AddWarning(
        ...
        self._warnings.append(
            Issue(message=message, severity=IssueSeverity.WARNING, rule=self.__class__, at=at, suggestion=suggestion)
        )
    ...
    def GetDescription(cls):
        ...
        if not cls.__doc__:
            return "Base description. This should be re-implemented by derived classes"
        lines = list(cls.__doc__.splitlines())
        ...
        for line in lines:
            ...
            if line_strip != "":
                indentation = len(line) - len(line_strip)
                if min_indentation is None or indentation < min_indentation:
                    min_indentation = indentation

        for line in lines:
            line_strip = line.lstrip()
            if line_strip != "" and min_indentation:
    ...
    def CheckDiagnostics(self, diagnostics):
        for diag in diagnostics:
            ...
            if '_ReportErrors' in diag.sourceFunction and \
                os.path.join('usd','stage.cpp') in diag.sourceFileName:
...
class TextureChecker(BaseRuleChecker):
    ...
    def __init__(self, verbose, consumerLevelChecks, assetLevelChecks):
        ...
        self._allowedFormats = list(TextureChecker._basicUSDZImageFormats) + TextureChecker._unsupportedImageFormats
    ...
    def CheckPrim(self, prim):
        ...
        if prim.GetTypeName() and connectable:
            if parent:
                ...
                if pConnectable and hasattr(pConnectable,'IsContainer') and not pConnectable.IsContainer():
...
class ARKitPackageEncapsulationChecker(BaseRuleChecker):
    """
    If the root layer is a package, then the composed stage should not contain references to files outside the
    package. In other words, the package should be entirely self-contained.
    """
...
class ARKitShaderChecker(BaseRuleChecker):
    """
    Shader nodes must have \"id\" as the implementationSource, with id values that begin with \"Usd*\". Also,
    shader inputs with connections must each have a single, valid connection source.
    """
...
class ARKitRootLayerChecker(BaseRuleChecker):
    """
    The root layer of the package must be a usdc file and must not include any external dependencies that participate
...
class ComplianceChecker(object):
    ...
    * "ARKit" compatibility validation, which includes many more restrictions.
    ...
    def DumpAllRules():
        print('Base rules:')
        for ruleNum, rule in enumerate(ComplianceChecker.GetBaseRules()):
            print('[%s] %s' % (ruleNum + 1, rule.GetDescription()))
        print('-' * 30)
        print('ARKit rules: ')
        for ruleNum, rule in enumerate(ComplianceChecker.GetARKitRules()):
            print('[%s] %s' % (ruleNum + 1, rule.GetDescription()))
        print('-' * 30)

    def __init__(self, arkit=False, skipARKitRootLayerCheck=False,
                 rootPackageOnly=False, skipVariants=False, verbose=False,
                 assetLevelChecks=True, initRules=False):
        ...
        if initRules:
            ...
            for Rule in ComplianceChecker.GetRules(arkit, skipARKitRootLayerCheck):
                self._rules.append(Rule(self._verbose, arkit, assetLevelChecks))

    def AddRule(self, Rule: BaseRuleChecker, consumerLevelChecks=False, assetLevelChecks=True):
        self._rules.append(
            Rule(
                verbose=self._verbose,
                consumerLevelChecks=consumerLevelChecks,
                assetLevelChecks=assetLevelChecks
            )
        )
    ...
    def _AddError(self, errMsg):
        self._errors.append(
            Issue(severity=IssueSeverity.ERROR, message=errMsg)
        )

    def _AddWarning(self, errMsg):
        self._warnings.append(
            Issue(severity=IssueSeverity.WARNING, message=errMsg)
        )

    def GetErrors(self):
        errors = self._errors
        for rule in self._rules:
            errors.extend(rule.GetErrors())
        return errors

    def GetWarnings(self):
        warnings = self._warnings
        for rule in self._rules:
            warnings.extend(rule.GetWarnings())
        return warnings
    ...
    def GetFailedChecks(self):
        failedChecks = []
        for rule in self._rules:
            failedChecks.extend(rule.GetFailedChecks())
        return failedChecks
    ...
    def CheckCompliance(self, inputFile):
        ...
        self.CheckStageCompliance(usdStage)

    def CheckStageCompliance(self, usdStage):
        ...
        with Ar.ResolverContextBinder(usdStage.GetPathResolverContext()):
            ...
            (allLayers, allAssets, unresolvedPaths) = \
                    UsdUtils.ComputeAllDependencies(Sdf.AssetPath(usdStage.GetRootLayer().identifier))
            for rule in self._rules:
            ...
            if self._rootPackageOnly:
                ...
                # Author all variant switches in a sublayer of the
                # session layer.
                complianceLayer = Sdf.Layer.CreateAnonymous("ComplianceChecker")
                usdStage.GetSessionLayer().subLayerPaths.insert(0, complianceLayer.identifier)
                originalTarget = usdStage.GetEditTarget()

                try:
                    usdStage.SetEditTarget(complianceLayer)
                    allPrimsIt = iter(Usd.PrimRange.Stage(usdStage,
                        Usd.TraverseInstanceProxies()))
                    self._TraverseRange(allPrimsIt, isStageRoot=True)
                finally:
                    usdStage.GetSessionLayer().subLayerPaths.remove(complianceLayer.identifier)
                    usdStage.SetEditTarget(originalTarget)
